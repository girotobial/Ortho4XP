...
class CleaningLevel(enum.IntEnum):
    """Cleaning level of log files"""
...
class Ortho4XP_Config(tk.Toplevel):
    def __init__(self, parent):
        ...
        for (title, sub_list) in (
            ...
            for item in sub_list:
                ...
                if (
                    cfg_vars[item]["type"] == bool
                    or "values" in cfg_vars[item]
                ):
                    values = (
                        [True, False]
    ...
    def load_tile_cfg(self):
        ...
        build_dir = filenames.build_dir(lat, lon, custom_build_dir)
        try:
            f = open(
                os.path.join(
                    build_dir,
                    "Ortho4XP_" + filenames.short_latlon(lat, lon) + ".cfg",
                ),
                "r",
            )
        except:
            try:
                ...
                self.popup(
                    "ERROR", "No config file found in " + str(build_dir)
                )
                return 0
        for line in f.readlines():
            line = line.strip()
    ...
    def load_global_cfg(self):
        try:
            f = open(os.path.join(filenames.Ortho4XP_dir, "Ortho4XP.cfg"), "r")
        except:
            return 0
        for line in f.readlines():
            line = line.strip()
    ...
    def write_global_cfg(self):
        old_cfg = os.path.join(filenames.Ortho4XP_dir, "Ortho4XP.cfg.bak")
        new_cfg = os.path.join(filenames.Ortho4XP_dir, "Ortho4XP.cfg")
