...
class Ortho4XP_Custom_ZL(tk.Toplevel):
    ...
    def __init__(self, parent, lat, lon):
        ...
        self.zl_choice.set(str(common.ZoomLevels.OSM_LEVELS[0]))
        ...
        self.zl_combo = ttk.Combobox(
            ...
            values=common.ZoomLevels.OSM_LEVELS,
            width=3,
            state="readonly",
            style="O4.TCombobox",
        )
        ...
        for col, btn_zl in enumerate(common.ZoomLevels.CUSTOM_LEVELS):
            btn = tk.Checkbutton(
                self.frame_zl_toggle_btn,
                bd=4,
                fg=common.ZoomLevels.tkinter_fg_color_of(btn_zl),
                bg=common.ZoomLevels.tkinter_color_of(btn_zl),
                activebackground=common.ZoomLevels.tkinter_color_of(btn_zl),
                selectcolor=common.ZoomLevels.tkinter_color_of(btn_zl),
                height=2,
                indicatoron=0,
                text="ZL" + str(btn_zl),
        ...
        ttk.Button(
            self.frame_left, text="Close Preview", command=self.destroy
        ).grid(row=row, column=0, padx=5, pady=3, sticky=N + S + E + W)
        row += 1
        ...
        self.apt_data_cache_banner.grid(
            row=right_row, column=0, sticky=N + S + W + E
        )
    ...
    def async_build_map_layer(lat, lon, zl, provider):
        try:
            ...
            drawer.rectangle(
                xy=[xy_top_left, xy_bottom_right], outline="black"
            )
        ...
        for zl in sorted(gtiles_dict.keys()):
            ...
            for texture_gtile in gtiles:
                ...
                drawer.rectangle(
                    ...
                    fill=common.ZoomLevels.rgba_color_of(texture_gtile.zl),
                    outline=common.ZoomLevels.rgba_border_color_of(
                        texture_gtile.zl
                    ),
                )
    ...
    def async_build_progressive_zl_layers(
    ...
    def async_build_dds_layout_layers(self, bg_map_lat, bg_map_lon, bg_map_zl):
        try:
            ...
            for dds_file in glob.glob(
                os.path.join(build_dir, "textures", "*.dds")
            ):
    ...
    def _async_render_layers(self, background_map_layer, texture_layers):
        try:
            ...
            self.canvas.create_image(
                0, 0, anchor=NW, image=layers["map"], tags="map"
            )
            ...
            if texture_layers is not None:
                ...
                for zl in sorted(texture_layers.keys()):
                    ...
                    layers[tag] = layer
                    self.canvas.create_image(
                        0, 0, anchor=NW, tags=tag, image=layer
                    )
            ...
            for zl in common.ZoomLevels.CUSTOM_LEVELS:
    ...
    def on_dsf_layout_button(self, lat, lon):
        ...
        # Asynchronously build the texture layers
        texture_layers = self.pool.submit(
            self.async_build_dsf_layout_layers,
            lat,
            lon,
            int(self.zl_combo.get()),
        )
    ...
    def redraw_poly(self):
        try:
            ...
            pass
        try:
            color = common.ZoomLevels.tkinter_color_of(self.zlpol.get())
...
class Ortho4XP_Earth_Preview(tk.Toplevel):
    ...
    def __init__(self, parent, lat, lon):
        ...
        self.frame_right.grid(
            row=0, rowspan=60, column=1, sticky=N + S + W + E
        )
                    ...
                    if os.path.isfile(
                        ...
                        if found_config:
                            ...
                            if not prov:
                                prov = "?"
                            if zl:
                                color = common.ZoomLevels.tkinter_color_of(zl)
                            else:
                                zl = "?"
                            content = prov + "\n" + str(zl)
                        else:
