...
class ZoomLevelsMeta(type):
    ...
    def __new__(mcs, *args, **kwargs):
        ...
        tkinter_fg_colors_dict = {
            zl: (
                "#000000"
                if mcs.__ZL_MIN__ <= zl <= mcs.__ZL_MAX__
                else "#FFFFFF"
            )
            for zl in range(mcs.__ZL_MIN__, mcs.__ZL_OVERKILL__ + 1)
        }
...
class ZoomLevels(metaclass=ZoomLevelsMeta):
    ...
    def normalized(cls, zl, min_zl=None, max_zl=None):
        ...
        return max(
            min(int(zl) + 1, max_zl or cls.__ZL_MAX__),
...
class CoverZLConfig:
    ...
    def __init__(self, cover_zl_value):
        if isinstance(cover_zl_value, int):
            ...
            raise ValueError(
                "Invalid CoverZL value : {}".format(cover_zl_value)
            )
        ...
        if isinstance(self._value, int):
            ...
            self.max = max(
                self.non_icao, self.icao, *list(self.by_icao.values())
            )
        else:
            raise Exception(
...
class DecalConfig:
    ...
    def __init__(self, config_value):
        if (
            ...
            try:
                ...
                if config_value == "True":
                    ...
                    self._value = {
                        int(zl): decal for (zl, decal) in dict_value.items()
                    }
        else:
            raise ValueError(
                "Invalid DecalConfig value : {}".format(config_value)
            )
        ...
        if isinstance(self._value, bool) and self._value is True:
            ...
            for zl in range(
                ZoomLevels.__ZL_HIGH__, ZoomLevels.__ZL_OVERKILL__ + 1
            ):
